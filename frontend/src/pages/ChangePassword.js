...
export default function ChangePassword() {
  ...
  if (!user.force_password_change) {
    const dest = user.role === 'platform_admin' ? '/platform-admin' : (getDefaultPathForRole(user) ?? '/');
      ...
      <div className="w-full max-w-md fade-in">
        ...
        <div className="rounded-2xl shadow-2xl p-8" style={{ background: '#ffffff', border: '1px solid #e8e0d4' }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-base font-semibold" style={{ color: '#625746', fontFamily: 'Montserrat, sans-serif' }}>Elige tu contraseña</h2>
              <p className="text-xs" style={{ color: '#b8a898', fontFamily: 'Montserrat, sans-serif' }}>Tu cuenta requiere crear una nueva contraseña</p>
            </div>
          </div>

          <div className="mb-5 px-3 py-2.5 rounded-lg" style={{ background: '#fdf8f2', border: '1px solid #e8dfd5' }}>
            <p className="text-xs" style={{ color: '#917a6a', fontFamily: 'Montserrat, sans-serif' }}>
              Sesión iniciada como{' '}
              <span className="font-semibold" style={{ color: '#625746' }}>{user.name}</span>
              {' · '}{user.email}
            </p>
          </div>
